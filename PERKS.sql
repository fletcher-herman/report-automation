...
SELECT lc.customer_id, date_from_f, date_to_f, insert_user, loyalty_code , gender,email_address
		 ,case when [birth_date] is NULL then 'Missing'
			...
			when datediff(year,cast([birth_date] as date),getdate())<=25 then '21-25'
			when datediff(year,cast([birth_date] as date),getdate())<=30 then '26-30'
			when datediff(year,cast([birth_date] as date),getdate())<=35 then '31-35'
			when datediff(year,cast([birth_date] as date),getdate())<=40 then '36-40'
			when datediff(year,cast([birth_date] as date),getdate())<=45 then '41-45'
			when datediff(year,cast([birth_date] as date),getdate())<=50 then '46-50'
			when datediff(year,cast([birth_date] as date),getdate())>51 then '51+'
